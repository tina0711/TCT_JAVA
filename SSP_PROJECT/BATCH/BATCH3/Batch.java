...
//			System.out.println(message);
			if(message.equals("P")){
				...
				if(arr[0].equals("C")){
					...
					if(batch.checkProgramCount(list) > 3){
						System.out.println("프로그램 실행 수 넘음");
	...
	public int checkProgramCount(List<Command> list) {
		int count = 0;
		...
		for(i=0; i<list.size(); i++) {
			...
			for(j=list.get(i).getGap(); j<20; j+=list.get(i).getGap()) {
				
				for(int k=j; k<j+list.get(i).getProgTime(); k++) {
